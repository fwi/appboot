 ...
 * <p>
 * The <code>appboot-demo</code> project contains examples on how AppBoot can be used and also contains Maven POM and assembly
 ...
 *</pre>To copy dependencies for an assembly build (configuration in assembler.xml): <pre>
 &lt;dependencySets&gt;
  &lt;dependencySet&gt;
    ...
    &lt;scope&gt;runtime&lt;/scope&gt;
    &lt;excludes&gt;
      &lt;exclude&gt;com.github.fwi:appboot&lt;/exclude&gt;
    &lt;/excludes&gt;
  &lt;/dependencySet&gt;
  &lt;dependencySet&gt;
    &lt;outputDirectory&gt;/lib&lt;/outputDirectory&gt;
    ...
    &lt;scope&gt;runtime&lt;/scope&gt;
    &lt;includes&gt;
      &lt;include&gt;com.github.fwi:appboot&lt;/include&gt;
    &lt;/includes&gt;
  &lt;/dependencySet&gt;
&lt;/dependencySets&gt;
 * </pre> 
...
public class AppBoot {
	...
	public static String getInfo() {
		...
		sb.append(CR).append("Parameters can be specified via a command-line argument, a Java system property (using the -D switch)");
		sb.append(CR).append("or an environment variable (uppercased with underscores, e.g. APP_NAME).");
		sb.append(CR).append("Double quotes surrounding parameter values are removed from the values.");
		...
		sb.append(CR).append("\tIf no name is set, AppBoot will use the name of the home-directory.");
		sb.append(CR).append("\tAppBoot will look for a jar-file starting with the application name.");
	...
	public static void main(String... args) {
		...
		AppBootDirs bootDirs = new AppBootDirs();
		bootDirs.getAppDirs(args);
		if (bootDirs.appHomeDir == null) {
			showInfo();
			return;
		}
		boolean appNameFromDir = false;
		if (mainClassName == null) {
			if (AppBoot.mavenTest) {
				throw new RuntimeException("Use of " + APP_MAIN + " is mandatory when " + APP_MAVEN_TEST + " is set.");
			}
			if (appName == null) {
				appName = bootDirs.appHomeDir.getName();
				appNameFromDir = true;
			}
		}
		List<File> appFiles = AppResource.getResources(bootDirs);
		if (mainClassName == null) {
			try {
		...
		for (File f : sysFiles) {
			if (appFiles.contains(f)) {
				appFiles.remove(f);
				if (debug) {
					showln("Removed system class-path: " + f);
				}
			}
		}
		...
		if (sysUrls != null) {
			for (URL u : sysUrls) {
				try {
					...
					// URI was not a file
					if (debug) {
						showln("Ignoring URL from system classloader: " + u);
					}
				}
			}
		}
...
--StartMode=jvm ^
--StartClass=com.github.fwi.appboot.AppBoot ^
++StartParams=-runAsService ^
--StopMode=jvm ^
--StopClass=com.github.fwi.appboot.AppBoot ^
--StopMethod=stop ^
...
pause</pre>
	...
	public static void stop(String[] args) {
		...
		if (mainClassName == null) {
			System.err.println("Cannot stop application service, application main class is unknown.");
		} else {
